...
class ProfileController extends Controller
{
    ...
    public function store(ProfileRequest $request)
    {
        ...
        ($request->exists('permissions')) ? $newProfile->permissions()->attach($data['permissions']) : $newProfile->permissions()->attach(1);
    ...
    public function update(ProfileRequest $request, Profile $profile)
    {
        $data = $request->all();
        $profile->update($data);
        ($request->exists('permissions')) ? $profile->permissions()->sync($data['permissions']) : $profile->permissions()->sync(1);
